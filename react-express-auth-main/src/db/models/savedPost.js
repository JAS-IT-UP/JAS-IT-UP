...
class SavedPost {
  ...
  static async list() {
    const query = 'SELECT * FROM saved_posts JOIN posts ON saved_posts.post_id = posts.id JOIN users ON saved_posts.user_id = users.id';
    const { rows } = await knex.raw(query);
    return rows.map((savedPost) => new SavedPost(savedPost));
  }

  static async find(id) {
    const query = 'SELECT * FROM saved_posts JOIN posts ON saved_posts.post_id = posts.id JOIN users ON saved_posts.user_id = users.id WHERE saved_posts.id = ?';
    const args = [id];
  ...
  static async findByUserId(userId) {
    const query = 'SELECT * FROM saved_posts JOIN posts ON saved_posts.post_id = posts.id JOIN users ON saved_posts.user_id = users.id WHERE saved_posts.user_id = ?';
    const args = [userId];
    const { rows } = await knex.raw(query, args);
    return rows.map((savedPost) => new SavedPost(savedPost));
  }
  ...
  update = async ({ post_id, user_id }) => { // dynamic queries are easier if you add more properties
